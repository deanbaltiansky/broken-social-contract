...
### Relative importance (Shapley)

We used relative importance (lmg/Shapley decomposition) to partition the explained variance in each outcome across the eight weighted value scores, while controlling for demographics and other covariates. This approach re-expresses the model’s R^2 in terms of the proportional contribution of each value, providing a clear indication of which values drive the overall effect of “broken promises.” By normalizing shares among the eight values, we highlight the relative weight of each value net of controls.
...
run_option_A <- function(outcome,
  ...
  # Relative importance (lmg) with CIs
  set.seed(seed)
  ri_boot <- boot.relimp(m_ols, b = boot_iter, type = "lmg", rela = TRUE)
  ri_ci   <- booteval.relimp(ri_boot)

  # Re-normalize to shares AMONG VALUES ONLY (controls remain in model)
  lmg_tbl <- tibble(
  ...
  values_lmg <- lmg_tbl %>% filter(predictor %in% value_cols)
  s <- sum(values_lmg$lmg)
  ri_values_table <- values_lmg %>%
    mutate(
      lmg_share_among_values = lmg / s,
      lmg_lo_among_values    = lmg_lo / s,
      lmg_hi_among_values    = lmg_hi / s
    ) %>%
  ...
  list(
    ...
    ri_values_table = ri_values_table   # % share among values (with CIs)
  )
}

res_A_antiest <- run_option_A("antiest_z", df_formodels, value_cols, control_vars, boot_iter = 1)
res_A_change  <- run_option_A("change_z", df_formodels, value_cols, control_vars, boot_iter = 100)
res_A_tdem    <- run_option_A("trust_deminst_z", df_formodels, value_cols, control_vars, boot_iter = 100)
res_A_tnat    <- run_option_A("trust_natinst_z", df_formodels, value_cols, control_vars, boot_iter = 100)

res_A_antiest$ri_values_table %>% print(n = 8)
```
